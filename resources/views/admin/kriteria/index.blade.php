<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            ...
            <a href="{{ route('admin.kriteria.create') }}" class="inline-flex items-center px-4 py-2 bg-emerald-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-emerald-700 transition duration-150">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="bg-white overflow-hidden shadow-sm border border-slate-200 rounded-lg">
                <div class="p-6">
                    ...
                    @if ($kriterias->isEmpty())
                        <div class="text-center py-20">
                            <div class="w-32 h-32 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-6">
                                <i class="fas fa-tasks text-5xl text-emerald-300"></i>
                            </div>
                                    ...
                                    @foreach ($kriterias as $kriteria)
                                        <tr class="hover:bg-emerald-50/50 transition duration-150">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <span class="px-2 py-1 inline-flex text-[10px] leading-4 font-bold rounded uppercase tracking-widest {{ strtolower($kriteria->tipe_faktor) === 'core' ? 'bg-slate-100 text-slate-700' : 'bg-emerald-50 text-emerald-700' }}">
                                                    {{ $kriteria->tipe_faktor }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-5 text-right whitespace-nowrap">
                                                <div class="flex justify-end gap-2">
                                                    <a href="{{ route('admin.kriteria.edit', $kriteria) }}" class="p-2 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-100 transition duration-300" title="Edit">
                    ...
                    <div class="mt-10 bg-emerald-50 p-6 rounded-2xl border border-emerald-100">
                        <h4 class="text-sm font-bold text-emerald-900 uppercase tracking-widest mb-2 flex items-center">
                            <i class="fas fa-info-circle mr-2"></i> Mengenal Tipe Faktor
                        </h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-emerald-800 leading-relaxed">
                            <div>
                                <span class="font-bold text-emerald-700">Core Factor:</span> Faktor yang paling penting dan utama dalam penilaian (misal: Kadar Purin).
                            </div>
                            <div>
                                <span class="font-bold text-emerald-700">Secondary Factor:</span> Faktor pendukung atau pelengkap dalam penilaian (misal: Protein, Lemak).
                            </div>
                        </div>
                    </div>
                </div>
