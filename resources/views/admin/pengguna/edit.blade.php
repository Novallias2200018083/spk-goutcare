        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="font-bold text-xl md:text-2xl text-slate-800 uppercase tracking-wide flex items-center">
                    <i class="fas fa-user-edit text-emerald-600 mr-3"></i> {{ __('Edit Data Pasien') }}
                </h2>
                <p class="text-[10px] md:text-xs text-slate-500 mt-1">Perbarui informasi akun dan kredensial login pasien.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative">
                ...
                <div class="p-6 md:p-8 relative z-10">
                    <form method="POST" action="{{ route('admin.pengguna.update', $pengguna) }}" x-data="{ metode: '{{ old('metode_input', $pengguna->profilPasien->metode_input ?? '') }}' }">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                                ...
                                <div x-show="metode === 'otomatis'" class="space-y-4 p-4 border border-slate-100 rounded-xl bg-slate-50/50" x-cloak>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select name="jenis_kelamin" class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                                ...
                                                <option value="L" {{ old('jenis_kelamin', $pengguna->profilPasien->jenis_kelamin ?? '') == 'L' ? 'selected' : '' }}>Laki-laki</option>
                                                <option value="P" {{ old('jenis_kelamin', $pengguna->profilPasien->jenis_kelamin ?? '') == 'P' ? 'selected' : '' }}>Perempuan</option>
                                            </select>
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Umur (Tahun)</label>
                                            <input type="number" name="umur" value="{{ old('umur', $pengguna->profilPasien->umur ?? '') }}" placeholder="Contoh: 45"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" step="0.1" name="berat_badan" value="{{ old('berat_badan', $pengguna->profilPasien->berat_badan ?? '') }}" placeholder="Contoh: 70"
                                                class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Tinggi Badan (cm)</label>
                                            <input type="number" step="0.1" name="tinggi_badan" value="{{ old('tinggi_badan', $pengguna->profilPasien->tinggi_badan ?? '') }}" placeholder="Contoh: 170"
                                                class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                        </div>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select name="tingkat_aktivitas" class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                                <option value="" disabled selected>Pilih...</option>
                                                <option value="rendah" {{ old('tingkat_aktivitas', $pengguna->profilPasien->tingkat_aktivitas ?? '') == 'rendah' ? 'selected' : '' }}>Rendah (Sedikit Gerak)</option>
                                                <option value="sedang" {{ old('tingkat_aktivitas', $pengguna->profilPasien->tingkat_aktivitas ?? '') == 'sedang' ? 'selected' : '' }}>Sedang (Olahraga Rutin)</option>
                                                <option value="tinggi" {{ old('tingkat_aktivitas', $pengguna->profilPasien->tingkat_aktivitas ?? '') == 'tinggi' ? 'selected' : '' }}>Tinggi (Fisik Berat)</option>
                                            </select>
                                        </div>
                                        <div>
                                            ...
                                            <select name="fase_asam_urat" class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                                <option value="" disabled selected>Pilih...</option>
                                                <option value="normal" {{ old('fase_asam_urat', $pengguna->profilPasien->fase_asam_urat ?? '') == 'normal' ? 'selected' : '' }}>Normal (Pemeliharaan)</option>
                                                <option value="akut" {{ old('fase_asam_urat', $pengguna->profilPasien->fase_asam_urat ?? '') == 'akut' ? 'selected' : '' }}>Akut (Sedang Kambuh)</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div x-show="metode === 'manual'" class="space-y-4 p-4 border border-slate-100 rounded-xl bg-slate-50/50" x-cloak>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" name="kebutuhan_kalori" value="{{ old('kebutuhan_kalori', $pengguna->profilPasien->kebutuhan_kalori ?? '') }}"
                                                class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Batas Purin (mg)</label>
                                            <input type="number" name="toleransi_purin" value="{{ old('toleransi_purin', $pengguna->profilPasien->toleransi_purin ?? '') }}"
                                                class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                        </div>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <input type="number" step="0.1" name="kebutuhan_protein" value="{{ old('kebutuhan_protein', $pengguna->profilPasien->kebutuhan_protein ?? '') }}"
                                                class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Lemak (g)</label>
                                            <input type="number" step="0.1" name="kebutuhan_lemak" value="{{ old('kebutuhan_lemak', $pengguna->profilPasien->kebutuhan_lemak ?? '') }}"
                                                class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Karbo (g)</label>
                                            <input type="number" step="0.1" name="kebutuhan_karbohidrat" value="{{ old('kebutuhan_karbohidrat', $pengguna->profilPasien->kebutuhan_karbohidrat ?? '') }}"
                                                class="w-full text-sm px-3 py-1.5 h-10 rounded-lg border-slate-200 bg-white focus:ring-1 focus:ring-indigo-500">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="p-5 md:p-6 bg-slate-50/50 rounded-2xl border border-slate-100 mt-8 relative overflow-hidden">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 relative z-10">
                                    <div>
                                        <label for="password" class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Password Baru <span class="text-slate-400 font-normal normal-case">(Opsional)</span></label>
                                        <div class="relative">
                                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
